...
def load_iEEG_macro(subject, session, folder='data/hiEEG'):
    ...
    *iEEG Macroelectrodes (Filtered to single-neuron) (4kHz)
    ...
    for trial_key in trial_keys:
        ...
        trial_meta = f['metadata']['Session']['sections']['Trial properties']['sections'][
            trial_key[len(trial_prefix) :]
        ]['properties']
        ...
        # Record
        data.append({'time': time, 'waveform': waveform, 'electrodes': electrode_names})
        meta.append(
            {
                'trial': None,
                'set_size': set_size,
...
def load_iEEG_micro(subject, session, folder='data/hiEEG'):
    ...
    *iEEG Microelectrodes/Depth electrodes (Filtered to single-neuron) (32kHz)
    ...
    micro_electrodes = (
        nwbfile.processing['ecephys'].data_interfaces['LFP'].electrical_series['ecephys.lfp'].electrodes[:]
    )
    reg_translation = {
        'Amyg': 'Amygdala',
        'FuG': 'Fusiform Gyrus',
    ...
    electrode_major_regions = np.array(
        [
            reg_translation[loc.split(', ')[0]] if loc.split(', ')[0] in reg_translation else loc.split(', ')[0]
            for loc in micro_electrodes['location'].to_numpy()
        ]
    )
    lateral = np.array(
        [
            loc.split(', ')[1].split(' ')[0] if len(loc.split(', ')) > 1 else None
            for loc in micro_electrodes['location'].to_numpy()
        ]
    )
    electrode_major_lateral_regions = np.array(
        [' '.join([s for s in (lat, loc) if s is not None]) for lat, loc in zip(lateral, electrode_major_regions)]
    )
    data = {
        'time': micro_time,
        'waveform': micro_data,
    ...
    micro_answers = (
        nwbfile.processing['behavior'].data_interfaces['BehavioralEvents.response'].time_series['response'].data[:]
    )
    micro_answers_time = (
        nwbfile.processing['behavior']
        .data_interfaces['BehavioralEvents.response']
